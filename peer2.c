...
write(sockfd,&pacchetto[0].porta,sizeof(pacchetto[0].porta));
char buffer[10];
read(sockfd,buffer,sizeof(buffer));
write(sockfd,&pacchetto,sizeof(pacchetto));


var = read(sockfd,&ricezione[index],sizeof(ricezione));

index += var/(sizeof(ricezione[0]));


for(int i = 0; i < 100; i++) {
if(strcmp(ricezione[i].porta,argv[1]) != 0 && strcmp(ricezione[i].porta,"") != 0)
printf("\n%d\n%s\n%s\n%s\n%s\n",i,ricezione[i].funzione,ricezione[i].porta,ricezione[i].parametri,ricezione[i].descrizione);
}
